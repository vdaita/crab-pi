...
pub fn matmul_func_test() {
    ...
    unsafe {
...

        {
            ...
            let mut c: [f32; M * N] = [0.0; M * N];
            let mut c_cpu: [f32; M * N] = [0.0; M * N];
            all_passed = run_matmul_case(gpu, "square-64", M, N, K, &a, &b, &mut c, &mut c_cpu) && all_passed;
        }

        {
            ...
            let mut c: [f32; M * N] = [0.0; M * N];
            let mut c_cpu: [f32; M * N] = [0.0; M * N];
            all_passed = run_matmul_case(gpu, "square-128", M, N, K, &a, &b, &mut c, &mut c_cpu) && all_passed;
        }
...

        {
            ...
            let mut c: [f32; M * N] = [0.0; M * N];
            let mut c_cpu: [f32; M * N] = [0.0; M * N];
            all_passed = run_matmul_case(gpu, "256x256x64", M, N, K, &a, &b, &mut c, &mut c_cpu) && all_passed;
        }
