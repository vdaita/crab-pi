...
// use crate::os::virtmem::{MemPerm, mmu_is_enabled, tlb_invalidate};
...
// pub fn print_program_context(ctx: &ProgramContext) {
//     println!(
//         "ProgramContext {{ user_stack: {:#x}, entry: {:#x}, arg0: {:#x}, arg1: {:#x}, arg2: {:#x} }}",
//         ctx.user_stack, ctx.entry, ctx.arg0, ctx.arg1, ctx.arg2
//     );
// }
...
//         self.pin_next(0x0000, 0x0100_0000, kern);
//         self.pin_next(0x0 + 16 * ONE_MB, 0x0100_0000 + 16 * ONE_MB, kern);
...
//         let elf_header_ptr: *mut holder::ElfHeader = (*file).data as *mut holder::ElfHeader;
//         let first_program_header_ptr: *mut holder::ProgramHeader =
//             ((*file).data as *mut u8).add((*elf_header_ptr).e_phoff as usize) as *mut holder::ProgramHeader;

//         println!("number of program headers: {}", (*elf_header_ptr).e_phnum);
//         println!("Program entry point (physical): {:#x}", (*elf_header_ptr).e_entry);

//         let program: &mut holder::Program = &mut *(0x0000_0000 as *mut holder::Program);
//         program.elf_header = *elf_header_ptr;

//          for prog_header_idx in 0..(*elf_header_ptr).e_phnum {
//             let program_header_ptr: *mut holder::ProgramHeader = first_program_header_ptr.add(prog_header_idx as usize);
...
//         let mapped_program = &mut *(0x0000_0000 as *mut holder::Program);

//         let program_addr = (mapped_program as *mut holder::Program) as usize;
...
//             // entry: (mapped_program.elf.data.as_mut_ptr()).byte_add((*elf_header_ptr).e_entry as usize) as u32,
//             arg0: 1,                                 // r0 = argc
...
//         // println!("After running breakpoint_mismatch_start()");
